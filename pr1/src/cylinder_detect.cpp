...
string calculatePoint(float distance, float angle, float x_cyl, float y_cyl)
{
  ...
  return "FOUND_CYLINDER " + to_string(X) + " " + to_string(Y)+  " " + to_string(angle+1.5) + " " + to_string(cylinder_array.size() - 1);
}


bool checkCylindersSimilarityForPossibles(Cylinder cilinder)
{
  //filterDoublePossibles();
  if(isnan(cilinder.x) || isnan(cilinder.y) || isnan(cilinder.z))
    return false;

  //if(possible_cylinders.size() == 1 && possible_cylinders[0].found == 3)
  //  return false;
  //else if(possible_cylinders.size() == 1 && possible_cylinders[0].found != 3)
  //  return true;
...
bool checkCylindersSimilarity(Cylinder cilinder)
{
  ...
  for (unsigned i=0; i < possible_cylinders.size(); i++) {
    if(possible_cylinders[i].sameCylinders(cilinder.x, cilinder.y, cilinder.z)){
      ...
      //possible_cylinders[i].found++;
      //std::cerr << "cylinder found " + to_string(possible_cylinders[i].found) + " times" << endl;
      if(possible_cylinders[i].found == 2){
        return true;
      }
    }
  }
...
cloud_cb (const pcl::PCLPointCloud2ConstPtr& cloud_blob)
{
  ...
  else
  {
          ...
          catch (tf2::TransformException &ex)
    {
        ...
        if(point_map.point.z > 0.1 && point_map.point.z < 0.25)
        {
          if(checkCylindersSimilarityForPossibles(cilinder))
        ...
        // nadaljuj, če je ta cilinder že bil 3x najden
