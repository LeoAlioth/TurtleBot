...
move_base_msgs::MoveBaseGoal get_next_pos(int &point, int &rot){
    ...
    if(rot == 0){
        point = (point + 1) % len;
        publishMessageToGoalPub("going_to");
    }
    return goal1;
}

move_base_msgs::MoveBaseGoal get_baseGoalMessage(float x, float y, float rot){
    tf::Quaternion q;
    q.setEuler(0, 0, rot);
    move_base_msgs::MoveBaseGoal goal1;
    goal1.target_pose.header.frame_id = "map";
    ...
    goal1.target_pose.pose.orientation.w = q[3];
    goal1.target_pose.pose.orientation.z = q[2];
    return goal1;
}
...
int main(int argc, char** argv){
    ...
    goal_pub = n.advertise<std_msgs::String>("goal_goal",1000);
